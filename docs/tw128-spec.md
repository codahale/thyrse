...
2. **Padded vs. unpadded.** Unpadded intermediate calls ($`\mathcal{U}`$) inherit capacity from prior $`\pi`$-outputs. So do non-init padded calls. Under $`\neg\mathsf{Bad}_{\mathrm{perm}}`$ these inherited capacities are pairwise distinct. Init calls start from zero capacity, which equals an inherited capacity with probability at most $`\sigma / 2^c`$, dominated by $`\varepsilon_{\mathrm{cap}}`$.
