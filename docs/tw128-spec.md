...
1. **Padded vs. padded (different domain bytes).** Each `pad_permute` call XORs its domain byte at the current offset `pos` in the rate. Two sub-cases:
   ...
   - **(b) Different `pos`.** By capacity-chain separation, the calls have distinct input capacities unless both start from zero capacity. The sole zero-capacity pair is init (`pos` = 40) vs. single-block KDF (`pos` = `len(X)`). The init rate contains $`K_{tw} = \pi(S_{\mathrm{kdf}})[0{:}32]`$ at bytes 0–31, where $`S_{\mathrm{kdf}}`$ is the KDF's pre-permutation state. Since $`\pi`$ is a uniformly random permutation, $`\pi(S_{\mathrm{kdf}})`$ is uniformly distributed over 1600 bits for any fixed $`S_{\mathrm{kdf}}`$. The probability that bytes 0–31 of this output equal the corresponding 32 bytes of the KDF rate is at most $`2^{-c}`$, dominated by $`\varepsilon_{\mathrm{cap}}`$. No assumption about the KDF $`\pi`$-input's distinctness from the init $`\pi`$-input is required.
