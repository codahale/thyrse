...
func TestKT128ReturnsXOF(t *testing.T) {
	_ = NewKT128()
	_ = NewKT128Custom([]byte("test"))
}

func TestKT128BlockSize(t *testing.T) {
	x := NewKT128()
	if x.BlockSize() != 8192 {
		t.Errorf("BlockSize() = %d, want 8192", x.BlockSize())
	}
}

func TestKT128Reset(t *testing.T) {
	x := NewKT128()
	_, _ = x.Write(ptn(100))
...
func TestKT128IncrementalRead(t *testing.T) {
	// One-shot read.
	x1 := NewKT128()
	_, _ = x1.Write(ptn(4913))
	want := make([]byte, 256)
	_, _ = x1.Read(want)

	// Incremental reads.
	x2 := NewKT128()
	_, _ = x2.Write(ptn(4913))
	var buf bytes.Buffer
	for _, s := range []int{1, 7, 16, 32, 64, 100, 36} {
		tmp := make([]byte, s)
		_, _ = x2.Read(tmp)
		buf.Write(tmp)
	}
...
func TestKT128CustomMatchesNew(t *testing.T) {
	// NewKT128Custom(nil) should behave identically to NewKT128().
	x1 := NewKT128()
	_, _ = x1.Write(ptn(100))
	out1 := make([]byte, 32)
	_, _ = x1.Read(out1)

	x2 := NewKT128Custom(nil)
	_, _ = x2.Write(ptn(100))
	out2 := make([]byte, 32)
	_, _ = x2.Read(out2)

	if !bytes.Equal(out1, out2) {
		t.Error("NewKT128Custom(nil) differs from NewKT128()")
	}
}
