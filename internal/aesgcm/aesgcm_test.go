...
// sealStdlib returns the expected ciphertext and tag built from the standard
// library, the oracle this package is validated against. The ciphertext is
// AES-CTR keystream from inc32(J0), obtained as the ciphertext half of a GCM
// seal; the tag is GMAC over that ciphertext, obtained by sealing an empty
// plaintext with the ciphertext as additional data. A nil nonce is treated as
// NonceSize zero bytes, matching this package.
func sealStdlib(t *testing.T, key, nonce, pt []byte) (ct, tag []byte) {
	t.Helper()
	...
	ct = aead.Seal(nil, nonce, pt, nil)[:len(pt)]
	tag = aead.Seal(nil, nonce, nil, ct)
	return ct, tag
}
...
func checkOneShot(t *testing.T, key, nonce, pt []byte) {
	t.Helper()
	wantCT, wantTag := sealStdlib(t, key, nonce, pt)
...
// TestStdlibCTRCompat checks the documented compatibility claim directly: for a
// 12-byte nonce the ciphertext equals crypto/cipher's AES-CTR with the initial
// counter block nonce || 0x00000002, and the tag equals stdlib GMAC over it.
func TestStdlibCTRCompat(t *testing.T) {
	key := fill(KeySize, 7)
	nonce := fill(NonceSize, 8)
	block, err := aes.NewCipher(key)
	if err != nil {
		t.Fatalf("aes.NewCipher: %v", err)
	}
	...
	iv := append(append([]byte(nil), nonce...), 0, 0, 0, 2)
	for _, n := range testSizes {
		pt := fill(n, int64(n))
		wantCT := make([]byte, n)
		cipher.NewCTR(block, iv).XORKeyStream(wantCT, pt)

		ct := make([]byte, n)
		tag := Encrypt(ct, key, nonce, pt)
		if !bytes.Equal(ct, wantCT) {
			t.Fatalf("n=%d ciphertext does not match stdlib AES-CTR\n got %x\nwant %x", n, ct, wantCT)
		}
		if wantTag := aead.Seal(nil, nonce, nil, ct); !bytes.Equal(tag, wantTag) {
			t.Fatalf("n=%d tag does not match stdlib GMAC\n got %x\nwant %x", n, tag, wantTag)
		}
	}
}
...
func TestUnverifiedPlaintext(t *testing.T) {
	...
	pt := fill(64, 13)
	ct, realTag := sealStdlib(t, key, nonce, pt)
...
// TestKAT checks known-answer vectors. The ciphertexts are CTR keystream
// vectors from the GCM specification (McGrew & Viega, Test Cases 1 and 2),
// which CTR+GMAC shares with GCM. The empty-message tag is also the spec's
// (with nothing to authenticate, GMAC and GCM coincide); the one-block tag is
// GMAC over the case-2 ciphertext, pinned from crypto/cipher's GCM
// (Seal(nil, nonce, nil, ct)), an implementation independent of this package.
func TestKAT(t *testing.T) {
	...
	cases := []struct {
		...
		{ // Test Case 2: one zero block.
			...
			tag: "74200c30691547de7551a4b3af27a2ed",
		},
	}
	for i, c := range cases {
		pt := dec(c.pt)
