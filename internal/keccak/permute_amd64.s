...
//
// AVX-512 x2 Keccak-p[1600,12]: deinterleaves State2 ([25][2]uint64) into
// two contiguous State1 buffers on the stack, permutes each using the x1
// round body (amortizing constant setup), then reinterleaves back.
TEXT ·p1600x2AVX512(SB), $400-8
	MOVQ	a+0(FP), BX

	// Deinterleave State2 (stride 16) → two contiguous State1 buffers.
	// State 0 → SP+0..SP+199, State 1 → SP+200..SP+399.
	LEAQ	0(SP), DI
	LEAQ	200(SP), SI
	XORQ	DX, DX
	MOVL	$25, CX
...
x2_deinterleave:
	...
	// Setup masks and load constants (once for both permutations).
	X1_SETUP_MASKS
	LEAQ	avx512_x1_consts(SB), R8
	X1_LOAD_CONSTS
...
x2_round_loop0:
	...
	// Permute state 1 (constants Z13-Z31 still loaded).
	LEAQ	200(SP), DI
	X1_LOAD_STATE
	LEAQ	avx512_x1_iotas+96(SB), R10
	MOVL	$6, AX
...
x2_round_loop1:
	...
	// Reinterleave: two State1 buffers → State2.
	LEAQ	0(SP), DI
	LEAQ	200(SP), SI
	XORQ	DX, DX
	MOVL	$25, CX
